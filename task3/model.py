...
class UNet(nn.Module):
    ...
    def __init__(self, in_channel, out_channel):
        ...
        self.conv_bottleneck = ConvBlock(MODEL_NEURONS * 8, MODEL_NEURONS * 16)
        ...
        self.conv_up1 = ConvBlock(
            MODEL_NEURONS * 2 + MODEL_NEURONS * 1, MODEL_NEURONS * 1
        )
    ...
    def forward(self, x):
        ...
        conv_u4 = self.conv_up4(torch.cat([self.upsamle(conv_b), conv_d4], dim=1))
        conv_u3 = self.conv_up3(torch.cat([self.upsamle(conv_u4), conv_d3], dim=1))
        conv_u2 = self.conv_up2(torch.cat([self.upsamle(conv_u3), conv_d2], dim=1))
        conv_u1 = self.conv_up1(torch.cat([self.upsamle(conv_u2), conv_d1], dim=1))
